...
namespace Com.TheFallenGames.OSA.Core
{
	...
	where TItemViewsHolder : BaseItemViewsHolder
	{
		...
		public virtual void OnInitializePotentialDrag(PointerEventData eventData)
		{
			...
			{
				float dtSinceLastOnEndDrag = Mathf.Max(Time.time - _LastOnEndDragTime, 0f);
		...
		public virtual void OnEndDrag(PointerEventData eventData)
		{
			...
			if (_Params.effects.inertia)
			{
				...
				float dtSinceLastOnDragStrictlyPositive = Mathf.Max(Time.time - _LastOnDragTime, 0f);
				float inertiaWaitTimeWindow = Mathf.Max(Time.deltaTime, .1f); // max time to wait for the inertia to still be applied
		...
		/// <summary> See <see cref="RequestChangeItemSizeAndUpdateLayout(int, float, bool, bool, bool)"/> for additional info or if you want to resize an item which isn't visible</summary>
		...
		public float RequestChangeItemSizeAndUpdateLayout(TItemViewsHolder withVH, float requestedSize, bool itemEndEdgeStationary = false, bool computeVisibility = true, bool correctItemPosition = false)
		{ return RequestChangeItemSizeAndUpdateLayout(withVH.ItemIndex, requestedSize, itemEndEdgeStationary, computeVisibility, correctItemPosition); }
		...
		public float RequestChangeItemSizeAndUpdateLayout(int itemIndex, float requestedSize, bool itemEndEdgeStationary = false, bool computeVisibility = true, bool correctItemPosition = false)
		{
			//double vsa = _InternalState.VirtualScrollableArea;
			//if (_Params.effects.loopItems)
			//{
